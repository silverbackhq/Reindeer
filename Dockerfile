# To build
# $ docker build -t clivern/reindeer:1.1.2 .
#
# To Run
# $ docker run -d \
#    --env APP_PORT=8000 \
...
COPY --from=builder /home/gradle/src/build/libs/reindeer-1.1.2-fat.jar /app/releases/reindeer-1.1.2-fat.jar
COPY --from=builder /home/gradle/src/.env.example /app/configs/.env

EXPOSE 8000

ENTRYPOINT ["java", "-jar", "/app/releases/reindeer-1.1.2-fat.jar", "--env=/app/configs/.env"]